...
| Archived | 已归档，保留于 Runtime Memory 供 Debug。不写入持久化存储。 | Released | N/A | N/A |
...
| Archived is the final state for all records | 所有终态记录最终进入 Archived — 保留于 Runtime Memory 供 Debug，不写入持久化存储。 |
...
| Previous Record MUST be terminal before retry | 在创建重试 Action Record 之前，前一个 Action Record **必须**已进入终态（Completed / Cancelled / Archived）。这保证了「Exactly One Active Action Record」不变量在重试场景下仍然成立。 |
