...
Action Execution Model is the **first Runtime Behavior Specification** in the Engine. Previous documents (Character State Schema, Relationship State Schema, Event Object Schema, SimulationResult Schema, Action Object Schema) defined *what data exists*. This document defines *how the Runtime behaves*.

Action Execution Model 是引擎中**第一份运行时行为规范**。之前的文档定义了*存在什么数据*。本文档定义*运行时如何行为*。
...
| Rule | Description |
|------|-------------|
| Exactly one active Action Record per Action | 一个 Action 在运行期间最多对应一个活跃的 Action Record。Replay 创建新的 Action Record，但原 Record 必须先归档。 |
| Action Record is mutable | Action Record 是可变的 — 它的职责就是追踪执行状态的变化。 |
| Action Record is transient | Action Record 是瞬态的 — Scene 结束后归档或销毁。Replay 时从 Action Object 重新创建。 |
| Action Record never modifies Action | Action Record 永远不修改关联的 Action Object。它只追踪执行状态。 |
| Action Record is not persisted | Action Record 不持久化 — 它是运行时对象，Scene 结束后丢弃。归档的只有 Action Object。 |

> **Exactly One Invariant:** At any point in time, at most one **active** Action Record exists for a given Action. An Action Record is "active" when its `queue_state` is one of: `created`, `queued`, `reserved`, `executing`. Once an Action Record reaches a terminal state (`completed`, `cancelled`, `archived`), it is no longer active, and a new Action Record may be created for the same Action (e.g., during Replay). This prevents concurrent execution tracking for the same Action.
...
| State | Description | Has Reservation? | Submitted to Simulation? | Produces Result? |
|-------|-------------|------------------|--------------------------|------------------|
| Created | Action Record 刚创建，尚未入队 | No | No | No |
| Queued | 已入队，等待调度 | No | No | No |
| Reserved | 资源已预留，等待执行 | Yes | No | No |
| Executing | 已提交到 Simulation Layer | Yes | Yes | In progress |
| Completed | SimulationResult 已产出 | Released | Yes | Yes |
| Cancelled | 被取消，不产出 SimulationResult | Released | No | No |
| Interrupted | 被中断，等待恢复或取消 | Held | Maybe | No |
| Archived | 已归档，保留用于 Debug | Released | N/A | N/A |

### Lifecycle Rules（生命周期规则）

| Rule | Description |
|------|-------------|
| Terminal states are irreversible | Completed / Cancelled / Archived 是终态，不可回退。 |
...
| Reservation released on terminal | 进入终态时（Completed / Cancelled），预留资源必须释放。 |
| Archived is the final state for all records | 所有终态记录最终进入 Archived，用于 Debug 和审计。 |
...
| Rule pre-check | 前置规则是否满足（如角色是否清醒） | Execution Authority |
| Simulation feasibility | 模拟是否可行（如技能冷却） | **Not** Execution Authority — Simulation Layer decides |

> **Validation vs Simulation:** Execution Authority validates *can this Action be submitted?* Simulation Authority decides *what happens when it's submitted?* An Action may pass Validation but produce a `failure` SimulationResult. An Action that fails Validation never reaches Simulation.
...
- **Terminal States are Irreversible:** Once Completed or Cancelled, an Action Record cannot return to a non-terminal state.
...
- **Reservation Always Released:** Resources are released when Action Record enters any terminal state.
...
| Stage | Description |
|-------|-------------|
| Draft → RC | 当 Action Record 结构、Lifecycle 状态机、Queue 规则经过 Review 后稳定 |
