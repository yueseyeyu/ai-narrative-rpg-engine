...
| category | 大类标签（用于检索过滤，如 "relationship_milestone", "world_change", "combat_resolution"） | Immutable |
...
| observer_ids | 观察者 ID 列表（在 Commit 时感知到事件的角色） | Immutable |
...
| Observer | 在场并感知到事件的角色 | 旁观者、目击者 |
| Affected | 状态被 Delta 修改的实体 | HP 减少的角色、物品栏变化的角色、标志变更的世界 |

> **Observer ≠ Affected:** A character may observe an event without being affected (a witness watching a fight), and a character may be affected without observing (a sleeping character whose inventory is stolen). The two lists are independent.
>
> **Empty Observers:** `observer_ids` may be empty — this means the event occurred but no one perceived it. This does NOT prevent the event from being committed. See §13 Runtime Guarantees.
...
Each Delta must contain four elements:
...
{
  "deltas": [
    { "target_id": "char_b", "op": "sub", "path": "stats.hp", "val": 35 },
    { "target_id": "char_a", "op": "transfer", "path": "inventory.sword_01", "val": "ground" },
    { "target_id": "world", "op": "set", "path": "flags.war_started", "val": true }
  ]
}
```
...
> **Visibility ≠ Memory Generation:** `visibility_scope` defines the *maximum* perception range. Memory generation requires a full perception pipeline: Event → Visibility Check → Observer Check → Perception Check → Memory Generation. A `public` event with `observer_ids=[]` (no witnesses) generates **zero** Memory Objects — but the world state still changes. See §13 Runtime Guarantees.
...
- **Committed Event Does Not Imply Memory Generation:** A Committed Event is objective reality — it changes World State regardless of whether anyone perceived it. Memory Generation requires a full perception pipeline:
  ...
  A `public` event with `observer_ids=[]` (secret murder, empty room) generates **zero** Memory Objects. But World State, Quest, and Relationship still update normally. This is a classic RPG design pattern — the world changes even when no one is watching.

  `public` 事件但 `observer_ids=[]`（秘密谋杀、空房间）生成**零** Memory Object。但 World State、Quest 和 Relationship 照常更新。
