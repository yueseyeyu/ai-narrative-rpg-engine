...
- **Self-Contained (Planner-Provided):** Action carries all Planner-provided information required by the Simulation Layer. It does not carry Runtime State, Snapshots, or Seeds — those are loaded separately by the Simulation Engine from the Runtime State Snapshot.
...
**Update Policy:** Changes affecting immutability rules, Field Admission Rule, Correlation Chain, Pure Input principle, or Parameters opacity require ADR approval.
